...
export class ChatService {
	...
	async createNewRoom(room: RoomDto): Promise<RoomDto> {
		...
		if (existingRoom && room.type !== RoomType.DIRECTMESSAGE) {
			...
			const newRoom = this.roomRepository.create({
				...
				type: room.type,
				password: room.password,//room.type === RoomType.PROTECTED ? await bcrypt.hash(room.password, 10) : null,
			});
	...
	async updateRoom(roomUpdate: RoomDto): Promise<RoomDto> {
		...
		room.password = roomUpdate.password;//roomUpdate.type === RoomType.PROTECTED ? await bcrypt.hash(roomUpdate.password, 10) : null;
